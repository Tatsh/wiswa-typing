...
Fixes mypy type errors and eliminates `Any` usage in the wiswa-typing project.
...
1. Fix all mypy errors reported by `uv run mypy wiswa.typing`
...
Place new TypedDict classes in `wiswa/typing/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`wiswa/typing/typing.py` when shared.
...
1. Run `uv run mypy wiswa.typing` and capture all errors.
...
4. Run `uv run mypy wiswa.typing` again to verify fixes.
