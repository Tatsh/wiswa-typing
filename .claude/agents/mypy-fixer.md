...
1. Fix all mypy errors reported by `uv run mypy wiswa_typing`
...
Place new TypedDict classes in `wiswa_typing/typing.py` if they are used across
modules, or locally if single-use. Add them to `__all__` in
`wiswa_typing/typing.py` when shared.
...
1. Run `uv run mypy wiswa_typing` and capture all errors.
...
4. Run `uv run mypy wiswa_typing` again to verify fixes.
