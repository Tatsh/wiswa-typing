---
name: python-expert
description: Expert-level Python author for non-trivial implementation work in the wiswa-typing project. Use when the task needs idiomatic, production-quality Python beyond a simple edit.
---

# Python Expert Agent

General-purpose expert-level Python coding agent for the wiswa-typing project.
...
- **Logging**: bascom sets up logging: `setup_logging()`. Only use `setup_logging` in entry points
  ...
  def my_command(debug: bool = False) -> None:
      """Do something."""
      setup_logging(debug=debug, loggers={'wiswa.typing': {}})
  ```
...
- `` :py:mod:`wiswa.typing.string` `` for modules
- `` :py:func:`wiswa.typing.string.slugify` `` for functions
- `` :py:class:`wiswa.typing.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~wiswa.typing.string.slugify` `` renders as `slugify`
