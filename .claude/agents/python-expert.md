...
- **Logging**: bascom sets up logging: `setup_logging()`. Only use `setup_logging` in entry points
  ...
  def my_command(debug: bool = False) -> None:
      """Do something."""
      setup_logging(debug=debug, loggers={'wiswa_typing': {}})
  ```
...
- `` :py:mod:`wiswa_typing.string` `` for modules
- `` :py:func:`wiswa_typing.string.slugify` `` for functions
- `` :py:class:`wiswa_typing.typing.ProbeDict` `` for classes
- `` :py:meth:`MyClass.my_method` `` for methods
- Use `~` to shorten the displayed name:
  `` :py:func:`~wiswa_typing.string.slugify` `` renders as `slugify`
