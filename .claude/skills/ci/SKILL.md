...
If any changed files are under `wiswa/typing/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `wiswa/typing/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `wiswa/typing/commands/main.py`,
...
For Python files, strip the `wiswa/typing/` prefix and replace `/` with `.` (like module imports).

- Python file `wiswa/typing/media.py` → `media:`.
- Multiple files under `wiswa/typing/commands/` → `commands:`.
- Single command file `wiswa/typing/commands/admin.py` → `commands.admin:`.
