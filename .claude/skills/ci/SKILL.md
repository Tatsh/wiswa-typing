...
If any changed files are under `wiswa_typing/` or `tests/`, run the following
agents **in order**:
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `wiswa_typing/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
...
For example, if a commit contains `wiswa_typing/commands/main.py`,
...
For Python files, strip the `wiswa_typing/` prefix and replace `/` with `.` (like module imports).

- Python file `wiswa_typing/media.py` → `media:`.
- Multiple files under `wiswa_typing/commands/` → `commands:`.
- Single command file `wiswa_typing/commands/admin.py` → `commands.admin:`.
